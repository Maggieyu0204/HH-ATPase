...
def NaKpump(points,t,para):
    [v, Nai,Ki,Nae,Ke,ATP,ADP,Pi,delta] = para #I(\muA/cm^2)
    [A, B, C, D] = points
...
J=[]
for Nae in [1.5,50,100,150,400]:
    J1=[]
    for v in np.arange(-120,60,5):
        # [v, Nai,Ki,Nae,Ke,ATP,ADP,Pi,delta]
        para = [v, 50, 140, Nae,5.4,9.8, 0.05, 4.2, delta]
        ...
        print(Nae,v,alpha2n,alpha2p,B,C)
        J1.append(Jp)
    J.append(J1)

x=np.arange(-120,60,5)
plt.plot(x,J[0],label='1.5')
plt.plot(x,J[1],label='50')
plt.plot(x,J[2],label='100')
plt.plot(x,J[3],label='150')
plt.plot(x,J[4],label='400')
plt.legend()
plt.show()